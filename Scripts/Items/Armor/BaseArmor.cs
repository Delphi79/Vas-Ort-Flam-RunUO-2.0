...
namespace Server.Items
{
	public abstract class BaseArmor : Item, IScissorable, IFactionItem, ICraftable, IWearableDurability
	{
        ...
        private List<Mobile> _IdentifiedTo;
        public List<Mobile> IdentifiedTo
        {
            get
            {
                return _IdentifiedTo;
            }
            set
            {
                _IdentifiedTo = value;
		...
		public override void Serialize( GenericWriter writer )
		{
            ...
            writer.WriteMobileList<Mobile>(_IdentifiedTo, true);
		...
		public override void Deserialize( GenericReader reader )
		{
            ...
            if (_IdentifiedTo == null)
                _IdentifiedTo = new List<Mobile>();
			...
			switch ( version )
			{
                case 8:
                    {
                        _IdentifiedTo = reader.ReadStrongMobileList();
                        goto case 7;
                    }
				case 7:
				case 6:
		...
		public override bool OnEquip( Mobile from )
		{
            ...
            if (AffectCharges > 0)
            {
                switch (MagicAffect)
                {
                    case PreUORMagAff.Agility:
                        {
                            Spells.Second.AgilitySpell spell = 
                                new Server.Spells.Second.AgilitySpell(from, 
                                new AgilityScroll());
                            spell.Target(from);
                            break;
                        }
                    case PreUORMagAff.Blessings:
                        {
                            Spells.Third.BlessSpell spell = 
                                new Server.Spells.Third.BlessSpell(from, 
                                new BlessScroll() );
                            spell.Target(from);
                            break;
                        }
                    case PreUORMagAff.Clumsiness:
                        {
                            Spells.First.ClumsySpell spell = 
                                new Server.Spells.First.ClumsySpell(from, 
                                new ClumsyScroll());
                            spell.Target(from);
                            break;
                        }
                    case PreUORMagAff.Cunning:
                        {
                            Spells.Second.CunningSpell spell = 
                                new Server.Spells.Second.CunningSpell( from, 
                                new CunningScroll());
                            spell.Target(from);
                            break;
                        }
                    case PreUORMagAff.Curses:
                        {
                            Spells.Fourth.CurseSpell spell = 
                                new Server.Spells.Fourth.CurseSpell(from, 
                                new CurseScroll());
                            spell.Target(from);
                            break;
                        }
                    case PreUORMagAff.Feeblemindedness:
                        {
                            Spells.First.FeeblemindSpell spell = 
                                new Server.Spells.First.FeeblemindSpell(from, 
                                new FeeblemindScroll());
                            spell.Target(from);
                            break;
                        }
                    case PreUORMagAff.Invisibility:
                        {
                            Spells.Sixth.InvisibilitySpell spell = 
                                new Server.Spells.Sixth.InvisibilitySpell(from, 
                                new InvisibilityScroll());
                            spell.Target(from);
                            break;
                        }
                    case PreUORMagAff.NightEyes:
                        {
                            Spells.First.NightSightSpell spell = 
                                new Server.Spells.First.NightSightSpell(from, 
                                new NightSightScroll());
                            spell.OnCast();
                            break;
                        }
                    case PreUORMagAff.Protection:
                        {
                            Spells.Second.ProtectionSpell spell = 
                                new Server.Spells.Second.ProtectionSpell(from, 
                                new ProtectionScroll());
                            spell.OnCast();
                            break;
                        }
                    case PreUORMagAff.SpellReflection:
                        {
                            Spells.Fifth.MagicReflectSpell spell = 
                                new Server.Spells.Fifth.MagicReflectSpell(from, 
                                new MagicReflectScroll());
                            spell.OnCast();
                            break;

                        }
                    case PreUORMagAff.Strength:
                        {
                            Spells.Second.StrengthSpell spell = 
                                new Server.Spells.Second.StrengthSpell(from, 
                                new StrengthScroll());
                            spell.Target(from);
                            break;

                        }
                    case PreUORMagAff.Weakness:
                        {
                            Spells.First.WeakenSpell spell = 
                                new Server.Spells.First.WeakenSpell(from, 
                                new WeakenScroll());
                            spell.Target(from);
                            break;
                        }
		...
		{
            double ar = (double)ArmorRating / (double)ArmorScalar;

            //double HalfAr = ar / 2.0;
            //int Absorbed = (int)Utility.RandomMinMax((int)HalfAr, (int)ar);
		...
		public override void OnSingleClick( Mobile from )
        {
            ...
            //string armorname = "";
            //if( number == 1025141 )
            //    armorname = "platemail chest";
            //else if( number == 1025199 )
            //    armorname = "bone chest";
            //else
            //    armorname = this.ItemData.Name;
            //if( eqInfo.Crafter != null )
            //{
            //    string crafter = eqInfo.Crafter.Name;
            //    armorinfo.AppendFormat( "an exceptional {0} crafted by {1}", armorname, crafter );
            //}
            //else if( m_Quality == ArmorQuality.Exceptional )
            //    armorinfo.AppendFormat( "an exceptional {0}", armorname );
            //else
            //    armorinfo.AppendFormat( "a {0}", armorname );
            //if( m_Identified || from.AccessLevel > AccessLevel.Player )
            //{
            //    string dur = "";
            //    string pro = "";
            //    string gram1 = "";
            ...
            //        gram2 = " and ";
            //    }
            //    armorinfo = new StringBuilder();
            //    if( m_PreUORMagAff != PreUORMagAff.None )
            //        armorinfo.AppendFormat( "a {0}{1}{2}{3}{4}{5}{6}", dur, armorname, gram1, pro, gram2, MagAffCharges, magaff );
            //    else
            //        armorinfo.AppendFormat( "a {0}{1}{2}{3}", dur, armorname, gram1, pro );
            //}
            ...
            //{
            //    string magic = " magic";
            //    armorinfo.Insert( 1, magic );
            //}

            //if( DisplayLootType )
            //{
            //    if( LootType == LootType.Blessed )
            //        armorinfo.Append( " [Blessed]" );
            //    else if( LootType == LootType.Cursed )
            //        armorinfo.Append( " [Cursed]" );
            //}
#endregion
            ...
            if (Quality != ArmorQuality.Regular)
            {
                if (m_Quality == ArmorQuality.Exceptional)
                    sb.Append(" exceptional");

                if (Name == null || Name == "")
                    sb.Append(" " + this.ItemData.Name);
                else
                    sb.Append(" " + Name);

                if (Crafter != null)
                    sb.AppendFormat(" crafted by {0}", Crafter.Name);
            }
            else
            {
                if (!_IdentifiedTo.Contains(from) && from.AccessLevel == AccessLevel.Player &&
                    ( Durability != ArmorDurabilityLevel.Regular || ProtectionLevel != ArmorProtectionLevel.Regular ) )
                    sb.Append(" magic");
                else
                {
                    switch (Durability)
                    {
                        case ArmorDurabilityLevel.Durable: sb.Append(" Durable"); break;
                        case ArmorDurabilityLevel.Fortified: sb.Append(" Fortified"); break;
                        case ArmorDurabilityLevel.Indestructible: sb.Append(" Indestructible"); break;
                        case ArmorDurabilityLevel.Massive: sb.Append(" Massive"); break;
                        case ArmorDurabilityLevel.Substantial: sb.Append(" Substantial"); break;
                        default: break;
                    }
                }
                ...
                if( _IdentifiedTo.Contains( from ) || from.AccessLevel != AccessLevel.Player )
                {
                    ...
                    {
                        case ArmorProtectionLevel.Defense: sb.Append(" of Defense"); break;
                        case ArmorProtectionLevel.Fortification: sb.Append(" of Fortification"); break;
                        case ArmorProtectionLevel.Guarding: sb.Append(" of Guarding"); break;
                        case ArmorProtectionLevel.Hardening: sb.Append(" of Hardening"); break;
                        case ArmorProtectionLevel.Invulnerability: sb.Append(" of Invulnerability"); break;
                        default: break;
                    }

                    if( AffectCharges > 0 && MagicAffect != PreUORMagAff.None )
                    {
                        switch (MagicAffect)
                        {
                            case PreUORMagAff.Agility: sb.Append(" and Agility"); break;
                            case PreUORMagAff.Blessings: sb.Append(" and Blessings"); break;
                            case PreUORMagAff.Clumsiness: sb.Append(" and Clumsiness"); break;
                            case PreUORMagAff.Cunning: sb.Append(" and Cunning"); break;
                            case PreUORMagAff.Curses: sb.Append(" and Curses"); break;
                            case PreUORMagAff.Feeblemindedness: sb.Append(" and Feeblemindedness"); break;
                            case PreUORMagAff.Invisibility: sb.Append(" and Invisibility"); break;
                            case PreUORMagAff.NightEyes: sb.Append(" and Night Eyes"); break;
                            case PreUORMagAff.Protection: sb.Append(" and Protection"); break;
                            case PreUORMagAff.SpellReflection: sb.Append(" and Spell Reflection"); break;
                            case PreUORMagAff.Strength: sb.Append(" and Strength"); break;
                            case PreUORMagAff.Weakness: sb.Append(" and Weakness"); break;
                            default: break;
                        }

                        sb.AppendFormat(" with {0} charges", AffectCharges);
                    }
                }            
            }
            ...
            switch (LootType)
            {
                case LootType.Blessed: sb.Append(" (blessed)"); break;
                case LootType.Cursed: sb.Append(" (cursed)"); break;
                default: break;
            }

            if (this.FactionItemState != null)
                sb.Append(" (factions)");
            ...
            this.LabelTo(from, "{0}", sb.ToString().ToLower());
		}
