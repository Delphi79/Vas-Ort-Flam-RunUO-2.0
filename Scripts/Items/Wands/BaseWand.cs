...
namespace Server.Items
{
	...
	public abstract class BaseWand : BaseBashing
	{
		...
		public virtual TimeSpan GetUseDelay{ get{ return TimeSpan.FromSeconds( 5.0 ); } }
		...
		public override void OnSingleClick( Mobile from )
        {
            ...
            if (Charges > 0 && Effect != null)
            {
                sb.Append(" magic wand" );

                if ( IdentifiedTo.Contains(from) || from.AccessLevel > AccessLevel.Player)
                {
                    if (Effect == WandEffect.GreaterHealing)
                        sb.Append(" of greater healing");
                    else if (Effect == WandEffect.MagicArrow)
                        sb.Append(" of magic arrow");
                    else if (Effect == WandEffect.ManaDraining)
                        sb.Append(" of mana drain");
                    else
                        sb.Append(" of " + Enum.GetName(typeof(WandEffect), Effect));

                    sb.AppendFormat(" with {0} charges", Charges);
                }
            }
            else
                sb.Append( " wand" );
