...
namespace Server.Items
{
	...
	public abstract class BaseWeapon : Item, IWeapon, IFactionItem, ICraftable, ISlayer, IDurability
	{
        ...
        [CommandProperty(AccessLevel.GameMaster)]
        public PreUORMagAff PreUORMagAff
        {
            get { return m_PreUORMagAff; }
            set { m_PreUORMagAff = value; InvalidateProperties(); }
        }
		...
		public virtual void OnHit( Mobile attacker, Mobile defender, double damageBonus )
		{
            ...
            if (PoisonCharges > 0 && Utility.RandomDouble() > .70)
            {
                defender.ApplyPoison(attacker, m_Poison);
                m_PoisonCharges--;
            }

            if (AffectCharges > 0 && Utility.RandomDouble() > .80)
            {
                switch (MagicAffect)
                {
                    case PreUORMagAff.Burning:
                        {
                            Spells.First.MagicArrowSpell spell =
                                new Server.Spells.First.MagicArrowSpell(attacker,
                                new MagicArrowScroll());
                            spell.Target(defender);
                            break;
                        }
                    case PreUORMagAff.Clumsiness:
                        {
                            Spells.First.ClumsySpell spell = 
                                new Server.Spells.First.ClumsySpell(attacker, 
                                new ClumsyScroll());
                            spell.Target(defender);
                            break;
                        }
                    case PreUORMagAff.DaemonBreath:
                        {
                            Spells.Third.FireballSpell spell =
                                new Server.Spells.Third.FireballSpell(attacker,
                                new FireballScroll());
                            spell.Target(defender);
                            break;
                        }
                    case PreUORMagAff.Evil:
                        {
                            Spells.Second.HarmSpell spell = 
                                new Server.Spells.Second.HarmSpell(attacker, 
                                new HarmScroll());
                            spell.Target(defender);
                            break;
                        }
                    case PreUORMagAff.Feeblemindedness:
                        {
                            Spells.First.FeeblemindSpell spell = 
                                new Server.Spells.First.FeeblemindSpell(attacker, 
                                new FeeblemindScroll());
                            spell.Target(defender);
                            break;
                        }
                    case PreUORMagAff.GhoulTouch:
                        {
                            Spells.Fourth.CurseSpell spell = 
                                new Server.Spells.Fourth.CurseSpell(attacker, 
                                new CurseScroll());
                            spell.Target(defender);
                            break;
                        }
                    case PreUORMagAff.MagesBane:
                        {
                            Spells.First.FeeblemindSpell spell = 
                                new Server.Spells.First.FeeblemindSpell(attacker, 
                                new FeeblemindScroll());
                    ...
                    case PreUORMagAff.Thunder:
                        {
                            Spells.Fourth.LightningSpell spell = 
                                new Server.Spells.Fourth.LightningSpell(attacker, 
                                new LightningScroll());
                            spell.Target(defender);
                            break;
                        }
                    case PreUORMagAff.Weakness:
                        {
                            Spells.First.WeakenSpell spell = 
                                new Server.Spells.First.WeakenSpell(attacker, 
                                new WeakenScroll());
                            spell.Target(defender);
                            break;
                        }
                    case PreUORMagAff.Wounding:
                        {
                            Spells.Second.HarmSpell spell = 
                                new Server.Spells.Second.HarmSpell(attacker, 
                                new HarmScroll());
		...
		public virtual void AddBlood( Mobile attacker, Mobile defender, int damage )
		{
			if ( damage > 0 )
			{
				...
				int extraBlood = (Core.SE ? Utility.RandomMinMax( 3, 4 ) : Utility.RandomMinMax( 3, 4 ) );
		...
		public virtual void GetBaseDamageRange( Mobile attacker, out int min, out int max, out int dicedamage )
		{
			if ( attacker is BaseCreature )
			{
			...
			min = 0;
			max = 0;
            dicedamage = WeaponControl.GetDamage(Damage);
		}

        public virtual string Damage { get { return "0d0+0"; } }
		...
		public virtual double GetBaseDamage( Mobile attacker )
		{
			...
			GetBaseDamageRange( attacker, out min, out max, out damagedice );
		...
		{
			int baseMin, baseMax, diceDamage;


            GetBaseDamageRange(from, out baseMin, out baseMax, out diceDamage);
		...
		public override void OnSingleClick( Mobile from )
        {
            ...
            */
            ...
            //        slayer, dur, gram1, gram2, acc, gram4, wepname, gram3, dam, and, magaff, with, MagAffCharges, poisoned, faction);
            //    else slayer, dur, gram1, gram2, acc, gram4, wepname, gram3, dam, poisoned, faction);
            //     
            #endregion
            ...
            StringBuilder sb = new StringBuilder();
                ...
                else
                {
                    if (!_IdentifiedTo.Contains(from) && from.AccessLevel == AccessLevel.Player && 
                        ( AccuracyLevel != WeaponAccuracyLevel.Regular || DurabilityLevel != WeaponDurabilityLevel.Regular || DamageLevel != WeaponDamageLevel.Regular ) )
                        sb.Append( " magic" );
                    else
                    {
                        if (this.Slayer == SlayerName.Silver)
                            sb.Append(" silver");

                        switch (DurabilityLevel)
                        {
                            case WeaponDurabilityLevel.Durable: sb.Append(" durable,"); break;
                            case WeaponDurabilityLevel.Fortified: sb.Append(" fortified,"); break;
                            case WeaponDurabilityLevel.Indestructible: sb.Append(" indestructible,"); break;
                            case WeaponDurabilityLevel.Massive: sb.Append(" massive"); break;
                            case WeaponDurabilityLevel.Substantial: sb.Append(" substantial,"); break;
                            default: break;
                        }

                        switch (AccuracyLevel)
                        {
                            case WeaponAccuracyLevel.Accurate: sb.Append(" accurate"); break;
                            case WeaponAccuracyLevel.Eminently: sb.Append(" eminently accurate"); break;
                            case WeaponAccuracyLevel.Exceedingly: sb.Append(" exceedingly accurate"); break;
                            case WeaponAccuracyLevel.Supremely: sb.Append(" supremely accurate"); break;
                            case WeaponAccuracyLevel.Surpassingly: sb.Append(" surpassingly accurate"); break;
                            default: break;
                        }
                    }
                        ...
                        ( AccuracyLevel != WeaponAccuracyLevel.Regular || DurabilityLevel != WeaponDurabilityLevel.Regular || DamageLevel != WeaponDamageLevel.Regular ) ) 
                    {
                        switch (DamageLevel)
                        {
                            case WeaponDamageLevel.Force: sb.Append(" of force"); break;
                            case WeaponDamageLevel.Might: sb.Append(" of might"); break;
                            case WeaponDamageLevel.Power: sb.Append(" of power"); break;
                            case WeaponDamageLevel.Ruin: sb.Append(" of ruin"); break;
                            case WeaponDamageLevel.Vanq: sb.Append(" of vanquishing"); break;
                            default: break;
                        }
                        ...
                        if (AffectCharges > 0 && MagicAffect != PreUORMagAff.None)
                        {
                            switch (PreUORMagAff)
                            {
                                case PreUORMagAff.Burning: sb.Append(" and Burning "); break;
                                case PreUORMagAff.Clumsiness: sb.Append(" and Clumsiness"); break;
                                case PreUORMagAff.DaemonBreath: sb.Append(" and Daemon's Breath"); break;
                                case PreUORMagAff.Evil: sb.Append(" and Evil"); break;
                                case PreUORMagAff.Feeblemindedness: sb.Append(" and Feeblemindedness"); break;
                                case PreUORMagAff.GhoulTouch: sb.Append(" and Ghoul's Touch"); break;
                                case PreUORMagAff.MagesBane: sb.Append(" and Mages Bane"); break;
                                case PreUORMagAff.Thunder: sb.Append(" and Thunder"); break;
                                case PreUORMagAff.Weakness: sb.Append(" and Weakness"); break;
                                case PreUORMagAff.Wounding: sb.Append(" and Wounding"); break;
                                default: break;
                            }
