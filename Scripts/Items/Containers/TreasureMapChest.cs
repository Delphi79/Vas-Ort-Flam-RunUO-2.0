			...
			else
			{
				...
				for ( int i = 0; i < level * 6; ++i )
				{
					...
					if ( item is BaseWeapon )
					{
						...
						{
							weapon.DamageLevel = (WeaponDamageLevel)Utility.Random( level+1 );
                            weapon.AccuracyLevel = (WeaponAccuracyLevel)Utility.Random(level + 1);
                            weapon.DurabilityLevel = (WeaponDurabilityLevel)Utility.Random(level + 1);
						}
					...
					else if ( item is BaseArmor )
					{
						...
						{
                            armor.ProtectionLevel = (ArmorProtectionLevel)Utility.Random(level + 1);
                            armor.Durability = (ArmorDurabilityLevel)Utility.Random(level + 1);
						}
