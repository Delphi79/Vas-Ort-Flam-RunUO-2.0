	...
	public class SkillCheck
	{
		...
		public static void Gain( Mobile from, Skill skill )
		{
            ...
            if (skill.Lock == SkillLock.Up)
            {
                ...
                if (from.StrLock == StatLockType.Up && from.Str < 100 && (info.StrGain / 8.33) > Utility.RandomDouble())
                    GainStat(from, Stat.Str);
                else if (from.DexLock == StatLockType.Up && from.Dex < 100 && (info.DexGain / 8.33) > Utility.RandomDouble())
                    GainStat(from, Stat.Dex);
                else if (from.IntLock == StatLockType.Up && from.Int < 100 && (info.IntGain / 8.33) > Utility.RandomDouble())
