...
namespace Server.Misc
{
	public class NotorietyHandlers
	{
		...
		public static bool Mobile_AllowBeneficial( Mobile from, Mobile target )
		{
            ...
            if (Server.Custom.PvpToolkit.PvpCore.IsInDeathmatch(from) || Server.Custom.PvpToolkit.PvpCore.IsInDeathmatch(target))
                return false;
        ...
        {            
            if (Server.Custom.PvpToolkit.PvpCore.IsInDeathmatch(source) && Server.Custom.PvpToolkit.PvpCore.IsInDeathmatch(target))
            {
                Server.Custom.PvpToolkit.DMatch.Items.DMStone dm1 = Server.Custom.PvpToolkit.PvpCore.GetPlayerStone(source);
                Server.Custom.PvpToolkit.DMatch.Items.DMStone dm2 = Server.Custom.PvpToolkit.PvpCore.GetPlayerStone(target);
            ...
            else if (Server.Custom.PvpToolkit.PvpCore.IsInDeathmatch(source) && !Server.Custom.PvpToolkit.PvpCore.IsInDeathmatch(target))
                return Notoriety.Invulnerable;
            else if (!Server.Custom.PvpToolkit.PvpCore.IsInDeathmatch(source) && Server.Custom.PvpToolkit.PvpCore.IsInDeathmatch(target))
                return Notoriety.Invulnerable;
