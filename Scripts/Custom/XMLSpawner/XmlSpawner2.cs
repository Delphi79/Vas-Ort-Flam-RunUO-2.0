...
namespace Server.Mobiles
{
    ...
    public class XmlSpawner : Item, ISpawner
    {
        ...
        public enum SpawnPositionType { Random, RowFill, ColFill, Perimeter, Player, Waypoint, RelXY, DeltaLocation, Location, Wet, Tiles }
        ...
        public const string Version = "3.14a";
        ...
        */
        ...
        public bool HasActiveSectors
        {
            get
            {
                ...
                if (m_Region != null)
                {
                    List<Mobile> p = m_Region.GetPlayers();
...
public static void _TraceEnd(int index)
{
	   if(index < MaxTraces){
        ...
        public static void Initialize()
        {
            ...
            CommandSystem.Register("XmlShow", AccessLevel.Seer, new CommandEventHandler(ShowSpawnPoints_OnCommand));
        ...
        public bool CheckRegionAssignment
        {
            ...
            set
            {
                if (value == true)
                {
                    ...
                    if (m_Region == null && m_RegionName != null)
                    {
                        RegionName = RegionName;
                    }
                }
        ...
        private void FindTileLocations(ref List<Point3D> locations, Map map, int startx, int starty, int width, int height, List<int> tiles, TileFlag tileflag)
        {
            ...
            for (int x = startx; x <= startx + width; x++)
            {
                for (int y = starty; y <= starty + height; y++)
                {
                    ...
                    // check the land tile
                    if ((tiles == null || tiles.Count == 0 || tiles.Contains(ltile.ID & 0x3FFF)) && ((TileData.LandTable[ltile.ID & 0x3FFF].Flags & tileflag) == tileflag))
                    {
                        //Console.WriteLine("found landtile {0} at {1},{2}", ltile.ID, x, y);
                    ...
                    for (int i = 0; i < statictiles.Length; ++i)
                    {
                        ...
                        if ((tiles == null || tiles.Count == 0 || tiles.Contains(stile.ID & 0x3FFF)) && ((TileData.ItemTable[stile.ID & 0x3FFF].Flags & tileflag) == tileflag))
                        {
                            //Console.WriteLine("found statictile {0} at {1},{2}", stile.ID, x, y);
        ...
        private void FindRegionTileLocations(ref List<Point3D> locations, Region r, List<int> tiles, TileFlag tileflag)
        {
            ...
            for (int n = 0; n < count; n++)
            {
                ...
                FindTileLocations(ref locations, r.Map, sx, sy, w, h, tiles, tileflag);
            }
        ...
        public Point3D GetSpawnPosition(bool requiresurface, int packrange, Point3D packcoord, List<SpawnPositionInfo> spawnpositioning)
        {
            ...
            if (tilelist != null || tileflag != TileFlag.None)
            {
                if (m_Region != null && HasRegionPoints(m_Region))
                {

                    FindRegionTileLocations(ref locations, m_Region, tilelist, tileflag);
                }
                else
                    if (positioning == SpawnPositionType.Random)
                    {
                        FindTileLocations(ref locations, this.Map, m_X, m_Y, m_Width, m_Height, tilelist, tileflag);
                    }
            }
        ...
        private class InternalTimer2 : Timer
        {
            ...
            protected override void OnTick()
            {
                if (m_Spawner != null && !m_Spawner.Deleted)
                {
                    ...
                    //m_Spawner.UpdateTotal(m_Spawner, TotalType.Items, -1);
                    m_Spawner.OnTick();
                }
